...
const COLUMN_HEADER_LABELS = {
  'Discharge to LDR': 'Discharge to LDR (AH - AE)',
  'LDR to Settlement': 'LDR to Settlement (AJ - AH)'
};
...
const getTATClaimVolumeByDays = (values) => {
  const maxPositiveValue = Math.max(0, ...values.filter(value => value >= 0));
  const weeklyBucketStarts = [0];
  for (let start = 8; start <= maxPositiveValue; start += 7) weeklyBucketStarts.push(start);

  const buckets = [
    { name: 'Data issue (< 0)', matches: value => value < 0 },
    ...weeklyBucketStarts.map(start => {
      const end = start === 0 ? 7 : start + 6;
      return {
        name: `${start}–${end} days`,
        matches: value => value >= start && value <= end
      };
    })
  ];
  ...
  return buckets.map(bucket => ({
    name: bucket.name,
    value: values.filter(bucket.matches).length
  }));
};
...
const calculateReimbTAT = (rows) => {
  ...
  return {
    ...
    dischargeToLDRClaimVolume: getTATClaimVolumeByDays(dischargeToLDRValues),
    ldrToSettlementClaimVolume: getTATClaimVolumeByDays(ldrToSettlementValues)
  };
};
...
const MISConverterTool = () => {
  ...
  const [isNegativeEndorsementPremium, setIsNegativeEndorsementPremium] = useState(false);
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      // TAT figure is derived from (e.g. "Discharge to LDR (AH - AE)"),
  ...
  const handleReset = () => {
    ...
    setIsNegativeEndorsementPremium(false);
    setReportDate('');
    setPolicyStartDate('');
    setPolicyEndDate('');
    setInceptionLives('');
    ...
    <div className="mis-tool-shell" style={styles.container}>
      ...
      <main className="mis-shell-main" style={styles.mainContent}>
              ...
              <>
                ...
                <div style={styles.section}>
                  ...
                  <label style={styles.inlineCheckboxLabel}>
                    <input
                      type="checkbox"
                      checked={isNegativeEndorsementPremium}
                      onChange={(e) => setIsNegativeEndorsementPremium(e.target.checked)}
                      style={{ cursor: 'pointer', width: '16px', height: '16px', flexShrink: 0 }}
                    />
                    <span>Make endorsement premium negative (return / refund premium)</span>
                  </label>
        ...
        {step === 'dashboard' && insightsRows && (() => {
          ...
          const signedEndorsementPremium = isNegativeEndorsementPremium && endorsementPremium !== ''
            ? -Number(endorsementPremium)
            : endorsementPremium;
          const lr = computeLossRatio({ inceptionPremium, endorsementPremium: signedEndorsementPremium, claimsIncurred, reportDate, policyStartDate, policyEndDate });
              ...
              <div ref={dashboardExportRef} style={{ backgroundColor: COLORS.bgElevated }}>
                ...
                <div style={styles.policyTotalsStrip} data-pdf-block="true">
                  ...
                  <div style={styles.policyTotalBox}>
                    ...
                    <div style={styles.policyTotalValue}>{endorsementPremium !== '' ? fmtCurrency(signedEndorsementPremium) : '—'}</div>
                  </div>
                  <div style={styles.policyTotalBox}>
                    ...
                    renderChart={(h) => (
                      a.reimbTAT.avgDischargeToLDR !== null ? (
                        <ResponsiveContainer width="100%" height={h}>
                          <BarChart
                            data={a.reimbTAT.dischargeToLDRClaimVolume}
                            layout="vertical"
                            margin={{ top: 5, right: 28, bottom: 24, left: 5 }}
                          >
                            <CartesianGrid strokeDasharray="3 3" stroke={COLORS.border} />
                            <XAxis type="number" allowDecimals={false} tick={{ fill: COLORS.textSecondary }} label={{ value: 'Number of claims', position: 'insideBottom', offset: -12, fill: COLORS.textSecondary, fontSize: 11 }} />
                            <YAxis type="category" dataKey="name" width={110} tick={{ fontSize: 11, fill: COLORS.textSecondary }} interval={0} label={{ value: 'TAT (days)', angle: -90, position: 'insideLeft', offset: 12, fill: COLORS.textSecondary, fontSize: 11 }} />
                            <Tooltip content={<WrappedBarTooltip />} cursor={{ fill: 'rgba(17,163,135,0.08)' }} />
                            <Bar dataKey="value" radius={[0, 4, 4, 0]} fill={COLORS.accent} maxBarSize={26}>
                  ...
                  <ChartCard
                    ...
                    renderChart={(h) => (
                      a.reimbTAT.avgLDRToSettlement !== null ? (
                        <ResponsiveContainer width="100%" height={h}>
                          <BarChart
                            data={a.reimbTAT.ldrToSettlementClaimVolume}
                            layout="vertical"
                            margin={{ top: 5, right: 28, bottom: 24, left: 5 }}
                          >
                            <CartesianGrid strokeDasharray="3 3" stroke={COLORS.border} />
                            ...
                            <YAxis type="category" dataKey="name" width={110} tick={{ fontSize: 11, fill: COLORS.textSecondary }} interval={0} label={{ value: 'TAT (days)', angle: -90, position: 'insideLeft', offset: 12, fill: COLORS.textSecondary, fontSize: 11 }} />
                            <Tooltip content={<WrappedBarTooltip />} cursor={{ fill: 'rgba(17,163,135,0.08)' }} />
                            <Bar dataKey="value" radius={[0, 4, 4, 0]} fill={COLORS.accentMid} maxBarSize={26}>
                              <LabelList dataKey="value" position="right" style={{ fontSize: 12, fontWeight: 700, fill: COLORS.textPrimary }} />
                            </Bar>
                          </BarChart>
                        </ResponsiveContainer>
        ...
        <div style={styles.infoBox}>
          ...
          <ul style={styles.infoList}>
            ...
            <li>The converted Excel now includes two extra blank columns — "Discharge to LDR (AH - AE)" and "LDR to Settlement (AJ - AH)" — for the team to fill in TAT (in days) manually, claim by claim. The header spells out which raw columns (by letter) each figure is derived from.</li>
