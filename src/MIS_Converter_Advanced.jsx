...
const MISConverterTool = () => {
  ...
  const handleInsightsFileUpload = async (e) => {
    ...
    try {
      ...
      // Do NOT auto-advance to the dashboard - stay on Step 3 so Total
      // Premium / Total Lives can be entered first.
    } catch (err) {
  ...
  const handleViewInsights = () => {
    ...
    if (totalPremium === '' || totalLives === '') {
      setInsightsFieldsError('Please enter both Total Premium and Total Number of Lives before viewing insights.');
      return;
    }
    ...
    <div className="mis-tool-shell" style={styles.container}>
        ...
        <div style={styles.card}>
          ...
          {insightsRows && (
            <>
              ...
              <div style={styles.section}>
                <div style={styles.sectionEyebrow}>Step 3 of 4</div>
                <h2 style={styles.sectionTitle}>Policy totals</h2>
                <p style={styles.description}>
                  Enter the total premium and total number of lives covered for this policy — these are
                  shown as widgets at the top of the insights dashboard, alongside the charts.
                </p>
                {insightsFieldsError && (
                  <div style={styles.errorBox}>
                    <AlertTriangle size={16} style={{ flexShrink: 0 }} />
                ...
                <div style={styles.fieldsRow}>
                  <div style={styles.fieldGroup}>
                    <label style={styles.fieldLabel}>Total premium (₹)</label>
                    <input
                      type="number"
                      min="0"
                      value={totalPremium}
                  ...
                  <div style={styles.fieldGroup}>
                    <label style={styles.fieldLabel}>Total number of lives</label>
                    <input
                      type="number"
                      min="0"
                      value={totalLives}
          ...
          <div style={styles.card}>
            ...
            <div style={styles.policyTotalsStrip}>
              <div style={styles.policyTotalBox}>
                ...
                <div style={styles.policyTotalValue}>
                  {totalPremium !== '' ? `₹${Number(totalPremium).toLocaleString('en-IN')}` : '—'}
                </div>
              </div>
              <div style={styles.policyTotalBox}>
                <div style={styles.policyTotalLabel}>Total number of lives</div>
                <div style={styles.policyTotalValue}>
                  {totalLives !== '' ? Number(totalLives).toLocaleString('en-IN') : '—'}
                </div>
              </div>
            </div>
            ...
            <div style={styles.dashboardGrid}>
              ...
              {/* 2. Document Receipt (FDR vs LDR) - three independent counts, NOT additive. */}
              <ChartCard
                title="Document receipt (FDR vs LDR)"
                renderChart={(h) => (
                  <ResponsiveContainer width="100%" height={h}>
                    <BarChart data={a.documentReceiptData} margin={{ top: 24, right: 5, left: 5, bottom: 5 }}>
              ...
              {/* 5. India map of claims by state */}
              <ChartCard
                title="Claims by state (India)"
                wide
                height={340}
                renderChart={(h) => (
                  <IndiaClaimsMap stateCounts={a.stateCounts} unmatchedCount={a.unmatchedStateCount} height={h} />
                )}
              />
      ...
      <div style={styles.infoBox}>
        ...
        <ul style={styles.infoList}>
          ...
          <li>Step 3 — upload the final file (with any team edits) to generate the insights dashboard: claim type, rejections, disease, relationship, document receipt (FDR/LDR), and a state-wise India map, all built from that uploaded file.</li>
...
const styles = {
  ...
  policyTotalsStrip: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '16px' },
  policyTotalBox: {
    backgroundColor: COLORS.mint,
